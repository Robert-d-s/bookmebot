...
export async function buildSystem(businessId: string): Promise<string> {
  ...
  return [
    ...
    `Opening hours: ${hours}.`,
    `Deposit refund cutoff: ${business.refundCutoffHours} hours before the appointment.`,
    "",
    "How to work:",
    ...
    "- Cancel or reschedule only the customer's own bookings (list_my_bookings first). Deposits are refunded only when cancelling at least the cutoff hours before the start; the cancel tool result says which.",
